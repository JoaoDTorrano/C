...
void cadastrarCidades(Cidade cidade[], int &totalCidades) {
    ...
    do {
        ...
        // Verificar se o nome já existe (comparação case-insensitive)
        for (int i = 0; i < totalCidades; i++) {
            string nomeExistente = cidade[i].nome;
            string novoNome = nome;
            
            transform(nomeExistente.begin(), nomeExistente.end(), nomeExistente.begin(), ::tolower);
            transform(novoNome.begin(), novoNome.end(), novoNome.begin(), ::tolower);
            
            if (nomeExistente == novoNome) {
                cout << "Erro: Ja existe uma cidade com este nome! Por favor, insira um nome diferente." << endl;
                nomeRepetido = true;
                break;
            }
        }
    ...
    do {
        ...
        for (int i = 0; i < totalCidades; i++) {
            if (cidade[i].codigo == codigo) {
                cout << "Erro: Ja existe uma cidade com este codigo! Por favor, insira um codigo diferente." << endl;
...
void cadastrarEstradas(Cidade cidade[], int totalCidades, int &totalEstradas) {
    ...
    // Verificar se já existe uma estrada entre essas cidades
    for (auto& estrada : cidade[idxOrigem].vizinha) {
        if (estrada.destino == destino) {
            cout << "Erro: Ja existe uma estrada entre estas cidades!" << endl;
            return;
        }
    }
